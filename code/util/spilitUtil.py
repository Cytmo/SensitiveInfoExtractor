...
class SensitiveInformation:
    ...
    def print_sensitive(self):
        ...
        for item in self.data_templete:
            templete_list = templete_list + sensitive_data_templete.get(str(item))
...
def process_shadow_file(filename):
    ...
    for line in shadow_file.readlines():
        ...
        if data_tmp[1].count('$')<4:
            sensitiveInformation.add_templete([2],[data_tmp[0],"伪用户或者无密码,该用户不能登录"])
        else:
            split_passwd = data_tmp[1].split("$")
            sensitiveInformation.add_templete([3],[data_tmp[0],shadow_passwd_type[split_passwd[1]],split_passwd[2],''.join(split_passwd[3:])])
        
        if data_tmp[3] == "0" or data_tmp[3] == "":
            sensitiveInformation.add_templete([4],[convert_format_time(int(data_tmp[2])),"无限"])
        else:
            sensitiveInformation.add_templete([5],[convert_format_time(int(data_tmp[2]))]+data_tmp[3:7])

        sensitiveInformation.print_sensitive()


process_shadow_file("/home/sakucy/networkCopitation/2023/data/linux/etc/shadow")
