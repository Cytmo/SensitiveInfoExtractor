...
class IoCIdentifier:
    ...
    def load_ioc_pattern(
        self,
        ioc_regexPattern_path: str = "./ioc_regexPattern.json",
        ioc_replaceWord: str = "./ioc_replaceWord.json",
    ):
    ...
    def ioc_replace(self):
        ...
        for ioc_item in self.ioc_list:
            ...
            replaced_word_start = (
                replaced_word_end - len(replaced_word) - 2
            )  # -2 for two blank space
            ...
            round_deleted_character_count = len(original_ioc_string) - len(
                replaced_word
            )
            ...
            logging.debug(
                ...
                % (
                    self.report_text[
                        ioc_item.ioc_location[0] : ioc_item.ioc_location[1]
                    ],
                    self.replaced_text[
                        replaced_ioc_item.ioc_location[
                            0
    ...
    def to_jsonl(self) -> str:
        ...
        for ioc_item in self.ioc_list:
            iocs.append(
                [ioc_item.ioc_location[0], ioc_item.ioc_location[1], ioc_item.ioc_type]
            )
    ...
    def check_replace_result(self):
        ...
        for replaced_ioc_item in self.replaced_ioc_list:
            replaced_string = self.replaced_text[
                replaced_ioc_item.ioc_location[0] : replaced_ioc_item.ioc_location[1]
            ]
            original_string = replaced_ioc_item.ioc_string
            print(
                "%d:%d:%s- %s"
                % (
                    replaced_ioc_item.ioc_location[0],
                    replaced_ioc_item.ioc_location[1],
                    replaced_string,
                    original_string,
                )
