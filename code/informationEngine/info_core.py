...
def marked_text_refinement(text: str) -> str:
    ...
    for key in PLACEHOLDERS_CORRESPONDING_TYPE:
        if PLACEHOLDERS_CORRESPONDING_TYPE[key][0][0] not in INFO_PATTERN:
            replaced_content = " {" + \
                PLACEHOLDERS_CORRESPONDING_TYPE[key][0][0] + "} " + key + " "
            logger.debug(
                TAG + 'marked_text_refinement(): Replacing {} with {}'.format(key, replaced_content))
            logger.debug(
                TAG + 'marked_text_refinement(): Text before replacement: '+str(text))
            text = text.replace(key, replaced_content)
            logger.debug(
                TAG + 'marked_text_refinement(): Text after replacement: '+str(text))
...
def code_info_extract(text: str) -> dict:
    ...
    text = '\n'.join(new_lines)
    logger.debug(
        TAG + 'code_info_extract(): text after removing outer " start:@@@ '+str(text)+' end:@@@')
    ...
    logger.debug(
        TAG + 'code_info_extract(): text after removing lines without string: '+str(text))
    text = text.split("\n")
    lines = []
    for line in text:
        line = line.strip()
...
def result_manager(result, info, file_path) -> dict:
    ...
    logger.info(
        TAG + "result_manager(): plain text info extract result: {}".format(str(result)))
    file_type = determine_file_type(file_path, info)
    switch = {
        'code': code_info_extract,
        'config': config_info_extract
